function [X,info] = IRcgls_chop(A,b,varargin)
% IRcgls Conjugate Gradient algorithm for Least Squares problems with chop
%
...
nrmb = vectorTwoNorm(b(:));
nrmAtb = vectorTwoNorm(d(:));
...
if isempty(NoiseLevel) || strcmp(NoiseLevel,'none')
    ...
    Rtol = chop(chop(eta)*chop(NoiseLevel));
end
...
if strcmp(x,'none')
    ...
    try
        ...
        sprintf("d now becomes %d", norm(d))
    catch
...
elseif TikParam == 0
    ...
    Lx = mv_blocked(L, TikParam*x);
    r = [ r ; -Lx ];
    d = chop(d - mv_blocked(L', TikParam*Lx));
end
...
if enrich
    ...
    S = zeros(n, q2);
    for i = 1:q2
        V(:,i) = mv_blocked(A, Q_tilde(:,i));
        S(:,i) = mv_blocked(A', V(:,i));
    end
...
if reorth, Q(:,1) = chop(d/vectorTwoNorm(d(:))); end
sprintf("before calculating normr2, d %d", norm(d))
normr2 = vv_blocked(d(:),d(:));
sprintf("normr2 %d", norm(normr2))
if precond
    d = P_solve(L, d);
end
if enrichTik
    s = chop(mv_blocked(A', r) - chop(chop(TikParam*TikParam)*x));
end
...
for k=1:MaxIter
    ...
    if tik
        Ad = [mv_blocked(A, d); mv_blocked(L, TikParam*d)];
    else
        Ad = mv_blocked(A, d);
        sprintf("norm(Ad) %d", norm(Ad))
    end
    if enrich
        f = mv_blocked(A', Ad);
    end
    normAd2 = vv_blocked(Ad,Ad);
    sprintf("norm(Ad2) %d", normAd2)
    if enrichTik
        alpha = chop(normr2/(chop(normAd2 + chop(chop(TikParam*TikParam)*(vv_blocked(d*d))))));
    else
    ...
    sprintf("r %d", norm(r))
    if tik
        s = chop(mv_blocked(A', r(1:m)) + mv_blocked(L', TikParam*r(m+1:end)));
    elseif enrichTik
        s = chop(s - chop(alpha*(chop(f + chop(chop(TikParam*TikParam)*d)))));
    else
        s  = mv_blocked(A', r);
        sprintf("s %d", norm(s))
    end
    ...
    if reorth
        for i=1:k, q = chop(q - chop(vv_blocked(Q(:,i),q)*Q(:,i))); end
        Q(:,k+1) = chop(q/vectorTwoNorm(q));
    end
    % normr2_new = norm(s)^2; %%% there was a bug!!!
    normr2_new = vv_blocked(q,q);
    if precond
        s = P_solve(L, q);
    else
        s = q;    
    ...
    if enrich
        if enrichTik
            g = chop((mv_blocked(V',Ad) + chop(chop(TikParam*TikParam)*mv_blocked(Q_tilde',d_old)))/...
                (chop(normAd2 + chop(chop(TikParam*TikParam)*vv_blocked(d_old,d_old)))));
        else 
           g = chop(mv_blocked(V',Ad)/normAd2);
        end
        Q_tilde  = chop(Q_tilde - mm_blocked(d_old,g'));
        V = chop(V - mm_blocked(Ad,g'));
        ...
        x_tilde = chop(x + mv_blocked(Q_tilde,y_tilde));
        r_tilde = chop(r - mv_blocked(V,y_tilde));
        S = chop(S - mm_blocked(f,g'));
        if enrichTik
            d_tilde = chop(d - mv_blocked(S + chop(chop(TikParam*TikParam)*Q_tilde),y_tilde));
        else
            d_tilde = chop(d - mv_blocked(S, y_tilde));
        end
    end
    
    % Compute norms.
    Xnrm(k)    = vectorTwoNorm(x(:));
    Rnrm(k)    = chop(vectorTwoNorm(r(:))/nrmb);
    NE_Rnrm(k) = chop(vectorTwoNorm(s(:))/nrmAtb);
    if enrich
        Xnrm_tilde(k)    = vectorTwoNorm(x_tilde(:));
        Rnrm_tilde(k)    = chop(vectorTwoNorm(r_tilde(:))/nrmb);
        NE_Rnrm_tilde(k) = chop(vectorTwoNorm(d_tilde(:))/nrmAtb);
    end
    if errornorms
        Enrm(k) = chop(vectorTwoNorm(chop(chop(x_true(:))-chop(x(:))))/nrmtrue);
        if Enrm(k)<BestEnrm
            BestReg.It = k;
            BestReg.X = x;
            BestReg.Xnrm = Xnrm(k);
        ...
        if enrich
            Enrm_tilde(k) = chop(vectorTwoNorm(chop(chop(x_true(:))-x_tilde(:)))/nrmtrue);
