function [X,info,lambda] = cs_chop(A,b,tol,varargin)
...
d = chop((chop(s_u^2+s_l^2))/2);
c = chop(chop(s_u^2-s_l^2)/2);
c
d
...
nrmAtb = norm(A'*b(1:m));
% nrmAtb = vectorTwoNorm(mv_blocked(A',b(1:m)));
nrmAtb
...
for k=1:min(num_iter,MaxIter)
    ...
    elseif k == 2
        cd = chop(c/d);
        beta = chop(1/2*chop(cd*cd));
        % alpha = chop(d-chop(chop(chop(c*c)/2)/d));
        alpha = chop(1/chop(d-chop(chop(chop(c/d)*c)/2)));
    else
        ac = chop(alpha*c);
        beta = chop(chop(ac/2)*chop(ac/2));
        alpha = chop(1/chop(d-chop(chop(ac*c)/4)));
    end

    % update v, x, and r
    % A'*r(1:ncols) + lambda*r(ncols+1:end)
    s = chop(mv_blocked(A',r(1:m))+chop(lambda*r(m+1:end)));
    v = chop(chop(beta * v) + s);

    % fprintf('norm of v is %4.2f',norm(v))
    
    % CHECK THIS FOR TIK:
    x = chop(x + chop(alpha*v));
    r = chop(r - chop(alpha * [mv_blocked(A,v); chop(lambda*v)]));
    
    % fprintf('norm of r is %4.2f',norm(r))
